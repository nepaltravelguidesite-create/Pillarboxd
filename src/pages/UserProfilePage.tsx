...
export function UserProfilePage({
  ...
  const avatarUrl =
    (isOwnProfile ? user?.avatarUrl : profile?.avatar_url) ?? undefined;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled && data) {
        setOwnProfileCounts(
          data as { follower_count: number; following_count: number },
        );
      }
    })();
    return () => {
  ...
  const [draftFavorites, setDraftFavorites] =
    useState<FavoriteShow[]>(favoriteShows);
  ...
  if (tab) {
    const TAB_CONFIG = {
      watchlist: {
        ...
        emptyDesc:
          "Browse shows and add them to your watchlist to keep track of what you want to watch next.",
      ...
      <div className="w-full max-w-screen-lg mx-auto px-4 py-4 space-y-5 md:py-8">
        ...
        <div className="flex items-start gap-4">
          ...
          {isOwnProfile ? (
            <Link
              to="/settings"
              className="p-2 rounded-md text-muted-foreground hover:text-foreground hover:bg-secondary/50 transition-colors shrink-0"
            >
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Start watching and logging shows to build your profile.
              </p>
            </div>
            <Link
              to="/shows"
              className="px-4 py-2 rounded-full bg-primary text-primary-foreground text-xs font-semibold hover:-translate-y-px hover:shadow-md hover:shadow-primary/25 transition-all"
            >
              Discover Shows
            </Link>
          </div>
    ...
    <div className="bg-card rounded-2xl p-4 border border-border/50 shadow-sm flex gap-4 mb-3 last:mb-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          {log.watched_date
            ? new Date(log.watched_date).toLocaleDateString(undefined, {
                year: "numeric",
                month: "short",
                day: "numeric",
              })
        ...
        {log.rating == null && log.vibe_tag && (
          <div className="mt-2">
            <VibeTagBadge value={log.vibe_tag} />
          </div>
        )}
          ...
          <p className="text-sm text-foreground/80 leading-7 mt-2">
            {log.contains_spoiler ? (
              <span className="text-xs text-muted-foreground italic">
                Contains spoilers. View on show page
              </span>
            ) : (
              log.review
            )}
    ...
    <Link to={`/show/${show.tmdb_id}`} className="block">
      <div className="aspect-poster rounded-lg overflow-hidden bg-muted border border-border/50">
        {show.poster_path && !imgError ? (
          <img
            src={bestPosterUrl(
              { id: show.tmdb_id, poster_path: show.poster_path },
              "w185",
            )}
            alt={show.name}
            loading="lazy"
